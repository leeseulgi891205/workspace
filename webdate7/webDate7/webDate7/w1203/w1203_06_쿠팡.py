...
for idx, name_div in enumerate(products, start=1):
    ...
    # 이미지 url 추출 (상품명 바로 위쪽 img 기준)
    img_tag = name_div.find_previous("img")
    if img_tag:
        image_url = img_tag.get("src") or img_tag.get("data-img-src") or img_tag.get("data-src")
    ...
    # 터미널에 출력
    print(f"[{idx}]")
    print("이미지 url :", image_url if image_url else "정보 없음")
    print("상품명 :", name)
...
with open(filename, "w", newline="", encoding="utf-8-sig") as f:
    writer = csv.writer(f)
    writer.writerow(["이미지url","상품명", "쿠폰", "정가", "할인가", "할인율(%)", "리뷰수", ])
