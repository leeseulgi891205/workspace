...
public class BoardServiceImpl implements BoardService {
	...
	public Map<String, Object> selectAll(int page) {
		// 1.하단넘버링에 필요한 컬럼
		// 총 게시글수 , 현재페이지, 최대페이지, 첫페이지, 마지막 페이지
		// page : 현재페이지
		int count = boardMapper.selectAllCount();
		double rowPerPage = 10; // 한페이지당 보여줄 게시글수
		// 102/10 = 10.2 -> 11페이지
		int maxPage = (int) Math.ceil(count / rowPerPage);
		int startPage = ((page - 1) / 10) * 10 + 1;
		int endPage = startPage + 10 - 1;
		if (endPage > maxPage) {
			endPage = maxPage; // 마지막페이지가 최대페이지보다 크면 마지막페이지는 최대페이지
		}
	...
	public BoardDto selectOne(BoardDto boardDto) {
		...
		// 이전글
		BoardDto prvedto = boardMapper.selectOnePrev(boardDto);
		// 다음글
