  ...
  <script>
    ...
    confirmDelete.addEventListener('click', () => {
      ...
      $.ajax({
         ...
         success: function(data){
             console.log(data);
             alert('게시글이 삭제되었습니다.');
             window.location.href = '/board/blist'; // 삭제 완료 후 이동하도록 success 안으로 위치 변경!
         },
         error: function(){
             alert("삭제 실패");
         }
        ...
        $(document).on("click", "#commentBtn", function(){
            ...
            $.ajax({
                ...
                success: function(data){
                    ...
                    `;
                    // AJAX 성공 시 화면에 댓글 부착 (위치 수정 완료)
                    $("#commentsList").append(html_data);

                    // 입력창 텍스트 비우기 (오타 수정 완료)
                    $('#commentText').val('');
                },
                error: function(){
                    alert("댓글 작성에 실패했습니다.");
    ...
    commentsList.addEventListener('click', (e) => {
      if (e.target.classList.contains('delete-btn')) {
        ...
        if (commentDiv.querySelector('textarea')) return; // 이미 수정 중이면 종료
