...
def home(request):
    ...
    if Report is not None:
        try:
            ...
            # 슬기 2025-12-24: 사건유형별 카운트
            crime_count = Report.objects.filter(created_at__gte=now - timedelta(days=7), category='violence').count()
            traffic_count = Report.objects.filter(created_at__gte=now - timedelta(days=7), category='traffic').count()
            disaster_count = Report.objects.filter(created_at__gte=now - timedelta(days=7), category='fire').count()
            other_count = Report.objects.filter(created_at__gte=now - timedelta(days=7)).exclude(category__in=['violence', 'traffic', 'fire']).count()
        except OperationalError:
            last7 = last30 = 0
            avg_risk_7 = 0
            crime_count = traffic_count = disaster_count = other_count = 0

    return render(request, 'pages/home.html', {
        'last7': last7, 
        'last30': last30, 
        'avg_risk_7': avg_risk_7,
