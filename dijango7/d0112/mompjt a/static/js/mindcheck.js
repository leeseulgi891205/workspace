document.addEventListener("DOMContentLoaded", () => {
  ...
  const elFill = document.getElementById("mindResultBarFill"); // 바 없으면 null이어도 OK
  ...
  function calcLevel(scores) {
    ...
    const avg = sum / scores.length; // 1 ~ 5
    ...
    function levelMeta(levelNum) {
        const map = {
            1: {
                emoji: "😔",
                sub: "지금은 정말 많이 버거운 상태예요.",
                msg: "하루 종일 참고 버티느라 힘들었죠? 혼자 감당하려 하지 않아도 괜찮아요. 비슷한 마음을 겪은 엄마들의 이야기를 먼저 읽어보세요. 말하지 않아도 위로가 될 수 있어요."
            },

            2: {
                emoji: "😕",
                sub: "조금씩 지치고 여유가 부족했을 수 있어요.",
                msg: "혹시 쉬고싶지 않나요? 지금 느끼는 감정을 짧게라도 꺼내놓으면 생각보다 가벼워질 수 있어요. 공감받는 글 하나만 읽어보세요."
            },

            3: {
                emoji: "😐",
                sub: "크게 흔들리진 않았지만 피로는 쌓여 있어요.",
                msg: "문제는 없지만 계속 이렇게 버티기만 하면 어느 순간 힘들어질 수 있어요. 오늘 하루 중 가장 힘들었던 순간 하나만 떠올려보세요. 그걸 적는 것만으로도 정리가 시작돼요."
            },

            4: {
                emoji: "🙂",
                sub: "오늘 하루, 꽤 잘 버텨냈어요.",
                msg: "완벽하지 않아도 충분히 잘하고 있어요. 오늘 내가 해낸 일 한 가지를 스스로 인정해 주세요. 그리고 여유가 된다면 다른 엄마의 글에 공감 버튼 하나 눌러보세요."
            },

            5: {
                emoji: "😊",
                sub: "마음이 비교적 안정적인 좋은 상태예요.",
                msg: "지금의 이 기분을 오래 가져가고 싶지 않나요? 오늘의 좋은 흐름을 기록으로 남겨보세요. 당신의 이야기가 누군가에게 큰 위로가 될 수 있어요."
            },
        };
