...
class FleaReport(models.Model):
    ...
    flea_item = models.ForeignKey(FleaItem, on_delete=models.CASCADE, related_name='reports', verbose_name='신고된 상품')
    reporter = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, null=True, blank=True, verbose_name='신고자')
    report_type = models.CharField(max_length=20, choices=REPORT_TYPE_CHOICES, default='etc', verbose_name='신고 유형')
    reason = models.TextField(blank=True, default='', verbose_name='신고 사유')
    ...
    def __str__(self):
        return f"{self.flea_item.title} - {self.get_report_type_display()}"
