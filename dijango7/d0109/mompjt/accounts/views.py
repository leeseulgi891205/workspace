    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if not user.check_password(password):
                ...
                return render(request, 'accounts/login/login.html', {'error': error_msg})
        except UserModel.DoesNotExist:
            error_msg = "아이디가 틀렸습니다."
            return render(request, 'accounts/login/login.html', {'error': error_msg})
        
        # 3. 로그인 처리
        login(request, user)
        messages.success(request, '정상적으로 로그인 되었습니다')
        return redirect('/')
    else:
...
def admin_dashboard(request):
    ...
    reports_pending = reports_range.filter(is_resolved=False).count()
    reports_resolved = reports_range.filter(is_resolved=True).count()
