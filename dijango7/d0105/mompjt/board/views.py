...
from .models import Notice, FreePost, FleaItem, FleaComment, Notification, FreePostReaction, HotDeal, ParentingInfo, PetPost, PetComment, MomDiary, DiaryEntry
...
# ═══════════════════════════════════════════════════════════════════════
# 이벤트/핫딜 게시판 (HotDeal) - 26-01-03 추가
# ═══════════════════════════════════════════════════════════════════════
...
def hotdeal_create(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            post.save()
            return redirect('board:hotdeal_detail', pk=post.pk)
    else:
        form = FleaItemForm()
    
    return render(request, 'board/hotdeal_form.html', {'form': form})
...
def hotdeal_update(request, pk):
    ...
    if request.method == 'POST':
        form = FleaItemForm(request.POST, request.FILES, instance=post)
        if form.is_valid():
            post = form.save()
            return redirect('board:hotdeal_detail', pk=post.pk)
    else:
        form = FleaItemForm(instance=post)
    
    return render(request, 'board/hotdeal_form.html', {'form': form, 'is_update': True})
...
def hotdeal_delete(request, pk):
    ...
    return render(request, 'board/hotdeal_delete.html', {'post': post})
