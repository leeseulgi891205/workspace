...
(function () {
  ...
  function showFleaOverlay(item, lat, lng) {
    ...
    // ✅ (요청) 제목/사진 키우고, 가격+거리 박스 하나로 합치기
    const content = `
      <div style="
        background:#fff; border-radius:14px; padding:10px 12px;
        box-shadow:0 12px 28px rgba(0,0,0,0.14);
        font-size:13px; line-height:1.35; min-width:280px;">
        
        <!-- 제목(크게) -->
        <div style="font-weight:900; font-size:17px; margin-bottom:10px; color:#212529;">
          ${title}
        </div>

        <div style="display:flex; gap:12px; align-items:stretch;">
          <!-- 사진(크게) -->
          <div style="width:120px; height:120px; border-radius:12px; overflow:hidden; background:#f1f3f5; flex:0 0 120px;">
            ${
              imgUrl
                ? `<img src="${imgUrl}" alt="" style="width:100%; height:100%; object-fit:cover; display:block;">`
                : `<div style="width:100%; height:100%; display:flex; align-items:center; justify-content:center; color:#adb5bd; font-weight:900; font-size:12px;">이미지 없음</div>`
            }
          </div>

        <!-- 가격 + 거리(하나의 박스 안에서 상/하 2줄) -->
        <div style="flex:1; display:flex; flex-direction:column; justify-content:flex-start;">
          <div style="
            border:1px solid #eee; border-radius:12px; padding:10px 12px;
            background:#fafafa; height:96px;
            display:flex; flex-direction:column;">

            <!-- 가격 줄(영역 중앙) -->
            <div style="flex:1; display:flex; align-items:center; gap:10px; line-height:1.2;">
              <div style="min-width:52px; color:#868e96; font-weight:900; font-size:13px;">
                가격
              </div>
              <div style="flex:1; text-align:right; color:#ff6b6b; font-weight:900; font-size:14px;">
                ${priceText || ""}
              </div>
            </div>
            ...
            <!-- 거리 줄(영역 중앙) -->
            <div style="flex:1; display:flex; align-items:center; gap:10px; line-height:1.2;">
              <div style="min-width:52px; display:flex; align-items:center; gap:6px; color:#868e96; font-weight:900; font-size:13px;">
                <img src="${routeIcon}" alt="" style="width:18px; height:18px; display:block;">
                거리
              </div>
              <div style="flex:1; text-align:right; color:#495057; font-weight:900; font-size:14px;">
                ${distValue || ""}
              </div>
            </div>
        ...
        <div style="display:flex; gap:10px; margin-top:12px;">
          <a href="${detailUrl}"
            style="flex:1; text-align:center; text-decoration:none; display:inline-flex;
              align-items:center; justify-content:center;
              border:0; border-radius:8px; padding:10px 10px;
              background:#ff6b6b; color:#fff; font-weight:900;">
            게시글 이동
          </a>

          <button id="lm_close_overlay"
            style="min-width:90px; border:1px solid #eee; border-radius:12px;
              padding:11px 10px; background:#fff; font-weight:900; cursor:pointer;">
            닫기
          </button>
        </div>
      </div>
    ...
    // 지도 영역 위에 모달 표시
    const existing = document.getElementById("lm_center_modal");
    if (existing) existing.remove();

    const modal = document.createElement("div");
    modal.id = "lm_center_modal";
    modal.style.cssText = `
      position: absolute;
      top: 0;
      left: 0;
    ...
    // 닫기 버튼
    const btn = dialog.querySelector("#lm_close_overlay");
  ...
  if (btnMyAddr) {
    btnMyAddr.addEventListener("click", function () {
      ...
      if (!addr && !detail) {
        setAddrText("등록된 회원 주소가 없어요. 마이페이지에서 주소를 등록해 주세요.");
        return;
      }
      
      // 주소 표시 (지오코딩 전에)
      const fullAddr = addr + (detail ? " " + detail : "");
      setAddrText(`회원 주소: ${fullAddr}`);
      ...
      setCenterFromUserAddress(
        ...
        () => {
          // 지오코딩 실패해도 주소는 그대로 표시
          setAddrText(`회원 주소: ${fullAddr} (지도 중심 이동 실패)`);
        }
      );
    });
  }
