...
def reply(request, bno):
    if request.method == 'GET':
        ...
        id = request.session.get('session_id')
        member = Member.objects.get(id=id)
        # 1. 답글달기 : 우선 같은 그룹에 있는 게시글의 bstep을 모두 +1
        # board_qs = Board.objects.filter(bgroup=bgroup, bstep_gt = bstep)
        # board_qs.update(bstep=F('bstep') + 1)
        
        
        
        # 답글 저장
        Board.objects.create(member=member,btitle=btitle,bcontent=bcontent,
            bfile=bfile,bgroup=parent_board.bgroup,bstep=parent_board.bstep + 1,
