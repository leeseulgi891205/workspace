...
# 경로 설정
original_db = r"c:\workspace\dijango7\d0102\mompjt\db.sqlite3"
modified_db = r"c:\workspace\dijango7\d0102\mompjt수정\db.sqlite3"
backup_db = r"c:\workspace\dijango7\d0102\mompjt\db.sqlite3.backup"

print("=" * 60)
print("SQLite 데이터베이스 병합 시작 (mompjt수정 → mompjt)")
print("=" * 60)

# 1단계: 원본 DB 백업
print("\n[1단계] mompjt의 db.sqlite3 백업 중...")
...
print(f"\n원본 DB (mompjt) 테이블: {len(original_tables)}개")
print(f"수정 DB (mompjt수정) 테이블: {len(modified_tables)}개")

# 3단계: 테이블별로 데이터 병합
print("\n[3단계] 데이터 병합 중...")

# 수정 DB의 데이터를 원본 DB에 추가
