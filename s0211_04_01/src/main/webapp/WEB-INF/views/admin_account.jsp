<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html lang="ko">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Document</title>
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.12.1/css/all.min.css">
  <link rel="stylesheet" href="css/admin_account.css">
</head>

<body>
  <header>
    <div id="header-left">
      LMS <span>ENTERPRISE</span> Type
    </div>
    <div id="header-right">
      <span>愿由ъ옄 (GMT+9:00) Seoul</span>
      <img src="http://lmsenterprise2019.itmap.co.kr/_admin/image/logout.png" alt="">

      <table>
        <tr>
          <td class="select">
            <img src="http://lmsenterprise2019.itmap.co.kr/_admin/image/KOR.png" alt="">
            <span>?쒓뎅??/span>
          </td>
          <td class="arrow">
            <div class="separator"></div>
            <span>??/span>
          </td>
        </tr>
      </table>

      <table>
        <tr>
          <td class="select">
            <span>LMS 留ㅻ돱??/span>
          </td>
          <td class="arrow">
            <div class="separator"></div>
            <span>??/span>
          </td>
        </tr>
      </table>
    </div>
  </header>

  <nav>
    <ul>
      <li><span class="nav1">&emsp;</span> ?뚯썝愿由?/li>
      <li><span class="nav2">&emsp;</span> ?곹뭹愿由?/li>
      <li><span class="nav3">&emsp;</span> 怨듭??ы빆</li>
      <li><span class="nav4">&emsp;</span> ?대깽??/li>
    </ul>
  </nav>

</body>
</html>
